var AShader = function(){
        ...
        this.fragmentShader = [
            ...
            "        gl_FragColor = mix(inputTex, col*0.98, dot(alpha.rgb, vec3(1.0))/3.0);",
            "    }",
            "    ",
...
var BShader = function(){
        ...
        this.fragmentShader = [
            ...
            "void main()",
            "{",
            "    const float ds = 2.1; // diffusion rate",
            ...
            "    ",
            "    vec3 im =    texture2D(BUF_B, vec2(mouse)*0.0025+vUv).xyz;",
            "    vec3 im_n =  texture2D(BUF_B, vec2(mouse)*0.0025+vUv+n).xyz;",
            "    vec3 im_e =  texture2D(BUF_B, vec2(mouse)*0.0025+vUv+e).xyz;",
            "    vec3 im_s =  texture2D(BUF_B, vec2(mouse)*0.0025+vUv+s).xyz;",
            "    vec3 im_w =  texture2D(BUF_B, vec2(mouse)*0.0025+vUv+w).xyz;",
            "    vec3 im_nw = texture2D(BUF_B, vec2(mouse)*0.0025+vUv+nw).xyz;",
            "    vec3 im_sw = texture2D(BUF_B, vec2(mouse)*0.0025+vUv+sw).xyz;",
            "    vec3 im_ne = texture2D(BUF_B, vec2(mouse)*0.0025+vUv+ne).xyz;",
            "    vec3 im_se = texture2D(BUF_B, vec2(mouse)*0.0025+vUv+se).xyz;",
